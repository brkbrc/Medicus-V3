...
public class AppData {
    ...
    public static DataSheet listDataDataSheet;

    public static DataSheet getDataSheetByName(String name) throws Exception {
        switch (name) {
            case "List data": return listDataDataSheet;
        }
    ...
    public static void loadDrawableFromUrl(final Context context, String url, final OnLoadingDrawableFinishedListener listener) {
        if (url.startsWith("asset://")) {
            ...
            listener.onLoadingDrawableFinished(resources.getDrawable(resId));
            return;
        }
            ...
            listener.onLoadingDrawableFinished(Drawable.createFromPath(path));
            return;
        }
                    ...
                    try {
                        ...
                        if (conn.getResponseCode() == HttpURLConnection.HTTP_OK) {
                            ...
                            new Handler(Looper.getMainLooper()).post(new Runnable() {
                                ...
                                public void run() {
                                    try {
                                        listener.onLoadingDrawableFinished(drawable);
                                    }
                                    catch (Exception e) {
                                        e.printStackTrace();
                                    }
                        ...
                        e.printStackTrace();
                        listener.onLoadingDrawableFinished(null);
                    }
                    return null;
                }
            }.execute(url);
    ...
    public interface OnLoadingDrawableFinishedListener {
        void onLoadingDrawableFinished(Drawable d);
    }
