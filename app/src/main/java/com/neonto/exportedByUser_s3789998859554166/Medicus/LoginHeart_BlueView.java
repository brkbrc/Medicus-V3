...
public class LoginHeart_BlueView extends FrameLayout {



    private ImageView m_image;
    // This value represents scaling applied to the image by Neonto Studio on export.
    private float m_contentOriginalScale = 0.287603306f;
    

    private boolean m_imageOverriden = false;

    public LoginHeart_BlueView(Context context) {
        super(context);
        init(null, 0);
        createViews(context);
    }
    
    public LoginHeart_BlueView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
        createViews(context);
    }
    
    public LoginHeart_BlueView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void createViews(Context context) {
        ...
        d = getResources().getDrawable(R.drawable.login_heart__blue);
